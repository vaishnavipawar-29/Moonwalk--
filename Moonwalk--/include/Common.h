...
#define MemCompare custom_memcmp
#define MemCopy custom_memcpy
// Using custom printf
...
#undef  malloc
#define malloc(x)   PICHeapAlloc(x)

#undef  realloc
#define realloc(x,s) PICHeapReAlloc(x, s)

#undef  free
#define free(x)     PICHeapFree(x)
...
void custom_printf(const char* pszFormat, ...) {
    ...
    va_end(argList);
    DWORD done;
    WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), buf, (DWORD)internal_strlen(buf), &done, NULL);
}
#else
void custom_printf(const char* pszFormat, ...) {};
#endif 
...
void* custom_memcpy(void* dest, void* src, unsigned int len)
{
    unsigned int i;
...
int custom_memcmp(const void* buf1, const void* buf2, unsigned int len)
{
    unsigned int i = 0;
...
int rand(unsigned long int next) // RAND_MAX assumed as 256 + 20
{
